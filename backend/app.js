...
app.get('/checkIfApptExists', (req, res) => {
  ...
  // Format the date to the correct MySQL format
  let ndate = new Date(date);
  let sql_date = `STR_TO_DATE('${ndate.getDate()}/${ndate.getMonth() + 1}/${ndate.getFullYear()}', '%d/%m/%Y')`;

  // Format the time for SQL
  let sql_start = `CONVERT('${startTime}', TIME)`;

  // Query to check if the patient has an existing appointment at the given time
  ...
  con.query(statement, function (error, results, fields) {
    if (error) {
      ...
      // Query to check if the doctor has an open appointment for the given time
      ...
      con.query(statement, function (error, results, fields) {
        if (error) {
          ...
          // Query to check if the doctor is available at the given time (not during a break)
          statement = `SELECT doctor, starttime, endtime, breaktime, day 
                       ...
                       AND (DATE_ADD(${sql_start}, INTERVAL +1 HOUR) <= breaktime 
          ...
          con.query(statement, function (error, results, fields) {
            if (error) {
              ...
              // If no schedule conflicts, set results to 1 (meaning available)
              if (results.length) {
                results = [];
              } else {
                results = [1];
              ...
              // Return the response by concatenating the data from all queries
              return res.json({
